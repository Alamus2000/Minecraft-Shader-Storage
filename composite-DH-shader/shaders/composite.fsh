...
void main() {
  ...
  float time_multi;
  if (worldTime >= 0.0f && worldTime <= 1000.0f) {
    time_multi = worldTime / 1000.0f; // Transition from 0 to 1
  } else if (worldTime > 1000.0f && worldTime < 12000.0f) {
    time_multi = 1.0f; // Fully enabled
  } else if (worldTime >= 12000.0f && worldTime <= 13000.0f) {
    time_multi = 1.0f - (worldTime - 12000.0f) / 1000.0f; // Transition from 1 to 0
  } else {
    time_multi = 0.1;
  }
	...
	vec3 sunlight = sunlightColor * clamp(dot(worldLightVector, normal), 0.0, 1.0) * shadow * time_multi;
